...
// serviceDeskBlock: общий «хвост» обоих сообщений — что делать и куда звонить.
const serviceDeskBlock = "Для продолжения использования удалённого доступа оформите заявку в Service Desk.\n" +
	"Название заявки «Удалённый доступ к стационарному АРМ для сотрудников Банка».\n" +
	"\n" +
	"Контакты Service Desk:\n" +
	"8-495-785-12-12, 055-5555 (Москва)\n" +
	"8-347-279-66-55 (Уфа)\n" +
	"8-800-200-01-55 (Все города)"

// certWarningBody: текст окна предупреждения. '\x01' обрамляет красные фрагменты
// (дата и слово «истёк»/«истечёт»). Отдельно для «скоро истечёт» и «уже истёк»
// (у истёкшего нет строки «Осталось» и фразы про «после этой даты»).
func certWarningBody(exp time.Time, name string, days int, expired bool) string {
	if expired {
		return fmt.Sprintf(
			"Ваш персональный сертификат для удалённого доступа \x01истёк\x01.\n\n"+
				"Владелец: %s\n"+
				"Срок действия: \x01%s\x01\n\n"+
				"%s",
			name, formatDate(exp), serviceDeskBlock)
	}
	return fmt.Sprintf(
		"Ваш персональный сертификат для удалённого доступа скоро \x01истечёт\x01.\n\n"+
			"Владелец: %s\n"+
			"Срок действия: \x01%s\x01\n"+
			"Осталось: %d %s\n\n"+
			"После этой даты удалённый доступ будет невозможен.\n\n"+
			"%s",
		name, formatDate(exp), days, pluralDays(days), serviceDeskBlock)
}
